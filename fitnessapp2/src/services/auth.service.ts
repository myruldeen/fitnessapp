...
export class AuthService {
	...
	register(user): Observable<Response> {
		return this.http.post(Globals.BASE_API_URL + 'api/users', user);
	}

	// Login User
	login(user): Observable<Response> {
		return this.http.post(Globals.BASE_API_URL + 'auth/signin', user);
	}
	...
	isAuthenticated(): Observable<boolean> {
		return new Observable<boolean>((observer) => {
			...
			this.http.get(Globals.BASE_API_URL + 'api/v1/users/me', {
				...
				this.toastService.toggleToast('Auto Login Failed!', 5000);
				this.setAuthState(false);
